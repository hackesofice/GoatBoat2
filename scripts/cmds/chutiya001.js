...
module.exports = {
	...
	onStart: async function ({ message, args, usersData, event, getLang }) {
		switch (args[0]) {
			...
			case "-a": {
				if (args[1]) {
					...
					for (const uid of uids) {
						if (scripts/cmds/001.allowedUsers.includes(uid))
							chutiyaIds.push(uid);
						else
							notChutiyaIds.push(uid);
					}

					scripts/cmds/001.allowedUsers.push(...notChutiyaIds);
			...
			case "-r": {
				if (args[1]) {
					...
					for (const uid of chutiyaIds)
						scripts/cmds/001.allowedUsers.splice(scripts/cmds/001.allowedUsers.indexOf(uid), 1);
			...
			case "list":
			case "-l": {
				const getNames = await Promise.all(scripts/cmds/001.allowedUsers.map(uid => usersData.getName(uid).then(name => ({ uid, name }))));
